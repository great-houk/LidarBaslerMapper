...
void Projector::project_points_pcl(const pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_cloud, cv::Mat &tvec, cv::Mat &rvec, cv::Mat &camera_mat, cv::Mat &distCoeffs, cv::Mat &output) {
...
void Projector::mapTurbo(double v, double vmin, double vmax, uint8_t &r, uint8_t &g, uint8_t &b) {
...
cv::Mat Projector::getProjectionImg(const cv::Mat &image, cv::Mat &tvec, cv::Mat &rvec, cv::Mat &camera_mat, cv::Mat &distCoeffs, const pcl::PointCloud<pcl::PointXYZI>::Ptr &lidar_cloud) {
    int width = image.cols, height = image.rows;
    cv::Mat projected_depths = cv::Mat::zeros(height, width, CV_64FC3);
    Projector::project_points_pcl(lidar_cloud, tvec, rvec, camera_mat, distCoeffs, projected_depths);
